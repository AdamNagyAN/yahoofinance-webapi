...
public class StockRepositoryImpl implements StockRepository {
  ...
  public List<StockCashflowStatement> getCashFlowStatements(String symbol) {
    String annualRequestUrl = UriComponentsBuilder.fromHttpUrl(cashFlowStatementApi).buildAndExpand(symbol).toUriString();
    String quarterlyRequestUrl = UriComponentsBuilder.fromHttpUrl(cashFlowStatementQuarterlyApi).buildAndExpand(symbol).toUriString();
    ...
    if ((annualResponse.getStatusCode() == HttpStatus.OK) && quarterlyResponse.getStatusCode() == HttpStatus.OK) {
      ...
      Optional<StockCashflowStatement> ttmCashFlowStatements = parseCashflowStatementsFromJson(quarterlyCashFlowStatements).stream().reduce(StockCashflowStatement::add);
  ...
  private List<StockCashflowStatement> parseCashflowStatementsFromJson(ArrayNode cashFlowStatements) {
    ...
    for (JsonNode item : cashFlowStatements) {
      ...
      StockCashflowStatement translatedObject = objectMapper.convertValue(jsonWithRawValues, StockCashflowStatement.class);
  ...
  public List<StockIncomeStatement> getIncomeStatements(String symbol) {
    String annualRequestUrl = UriComponentsBuilder.fromHttpUrl(incomeStatementsApi).buildAndExpand(symbol).toUriString();
    String quarterlyRequestUrl = UriComponentsBuilder.fromHttpUrl(incomeStatementsQuarterlyApi).buildAndExpand(symbol).toUriString();
    ...
    if ((annualResponse.getStatusCode() == HttpStatus.OK) && quarterlyResponse.getStatusCode() == HttpStatus.OK) {
      JsonNode annualJson = objectMapper.readTree(annualResponse.getBody());
      JsonNode quarterlyJson = objectMapper.readTree(quarterlyResponse.getBody());
      ArrayNode incomeStatements = (ArrayNode) annualJson.get("quoteSummary").get("result").get(0).get("incomeStatementHistory").get("incomeStatementHistory");
      ArrayNode quarterlyIncomeStatements = (ArrayNode) quarterlyJson.get("quoteSummary").get("result").get(0).get("incomeStatementHistoryQuarterly").get("incomeStatementHistory");

      Optional<StockIncomeStatement> ttmIncomeStatements = parseStockIncomeStatementsFromJson(quarterlyIncomeStatements).stream().reduce(StockIncomeStatement::add);
  ...
  public List<StockBalanceSheet> getBalanceSheets(String symbol) {
    String annualRequestUrl = UriComponentsBuilder.fromHttpUrl(balanceSheetApi).buildAndExpand(symbol).toUriString();
    String quarterlyRequestUrl = UriComponentsBuilder.fromHttpUrl(balanceSheetQuarterlyApi).buildAndExpand(symbol).toUriString();
    ResponseEntity<String> annualResponse = restTemplate.getForEntity(annualRequestUrl, String.class);
    ResponseEntity<String> quarterlyResponse = restTemplate.getForEntity(quarterlyRequestUrl, String.class);
    if ((annualResponse.getStatusCode() == HttpStatus.OK) && quarterlyResponse.getStatusCode() == HttpStatus.OK) {
      JsonNode annualJson = objectMapper.readTree(annualResponse.getBody());
  ...
  private List<StockIncomeStatement> parseStockIncomeStatementsFromJson(ArrayNode stockIncomeStatementJson) {
    ...
    for (JsonNode item : stockIncomeStatementJson) {
      Map<String, Long> jsonWithRawValues = extractRowValueFromEntries(item);
      StockIncomeStatement translatedObject = objectMapper.convertValue(jsonWithRawValues, StockIncomeStatement.class);
